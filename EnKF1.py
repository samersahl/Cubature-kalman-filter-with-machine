...
P= pd.read_excel('PP1.xlsx', header = None)
...
def build_multi_conv1D_model():
    ...
    cnn3 = keras.layers.Conv1D(40, 5, padding="same", activation="relu", strides=1)(cnn2)
    cnn4 = keras.layers.Conv1D(1, 1, padding="same", activation="linear", strides=1,
                               kernel_regularizer=keras.regularizers.L2(.0001))(cnn3)
    add = keras.layers.Add()([cnn4, input])
...
print(plot_model(model_conv1D_2, to_file='model_plot1.png', show_shapes=True, show_layer_names=True))
...
io.savemat('x2.mat', {"data1":  prediction_whole_model})
fig.show()
