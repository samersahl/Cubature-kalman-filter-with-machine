...
data_train = pd.read_excel('state3.xlsx', header=None)
correct_train = pd.read_excel('obser1.xlsx', header=None)
P= pd.read_excel('PP3.xlsx', header = None)
...
def build_multi_conv1D_model():
    ...
    cnn1 = keras.layers.Conv1D(10, 5, kernel_initializer='normal',
    ...
    cnn2 = keras.layers.Conv1D(20, 5, padding="same", activation="relu", strides=1)(conc)
    cnn3 = keras.layers.Conv1D(20, 5, padding="same", activation="relu", strides=1)(cnn2)
    cnn4 = keras.layers.Conv1D(15, 5, padding="same", activation="relu", strides=1)(cnn3)
    cnn5 = keras.layers.Conv1D(1, 1, padding="same", activation="linear", strides=1,
                               kernel_regularizer=keras.regularizers.L2(.0001))(cnn4)
    add = keras.layers.Add()([cnn5, input])
...
print(plot_model(model_conv1D_2, to_file='model_plot3.png', show_shapes=True, show_layer_names=True))
model_fit = model_conv1D_2.fit(train_data_reshaped, correct_data_reshaped,
                               ...
                               epochs=200, verbose=2, batch_size=8, validation_batch_size=2)
...
io.savemat('x4.mat', {"data1":  prediction_whole_model})
fig.show()
